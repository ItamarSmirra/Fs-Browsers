...
import { FILE_TYPES, ExportFileOptions } from '../types';

// ExcelService.wakeUp();

const getFileNameBySuffix = 
    (fileName: string, suffix: string) => fileName ? `${fileName.split('.')[0]}.${suffix}` : `file.${suffix}`
...
const exportCsvFile = (data, { fileName, headings }: { fileName: string, headings: string[] | null | undefined }) => {
...
const exportFile = (data: any, options: ExportFileOptions = { fileName: 'file.txt', type: FILE_TYPES.OTHER, headings: null }) => {
