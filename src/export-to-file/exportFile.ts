...
export const exportFile = (text: string, fileName: string = "file.txt") => {
...
export const exportXlsxFile = (
  ...
  const headingsRow =
    options && options.headings ? options.headings : Object.keys(data[0]);
  const cellStyle =
    options && options.cellStyle ? options.cellStyle : defaultCellStyle;
  const headingStyle =
    options && options.headingStyle
      ? options.headingStyle
      : defaultHeadingsStyle;
  const titleStyle =
    options && options.titleStyle ? options.titleStyle : defaultTitleStyle;
